...
#ifdef GBP_DEBUG
/// \brief "Print variable". Prints the text of an expression, followed by its value (only if GBP_DEBUG is defined)
/**
 *  Useful debugging macro to see what your code is doing.
 *  Example: \code GBP_PV(3+4) \endcode
...
/// "Debugging message": Prints a message (only if GBP_DEBUG is defined)
#define GBP_DMSG(str)                          \
        do                                     \
        {                                      \
...
namespace gbp
{
        ...
        {
                GBP_ASSERT(N <= (BigInt)std::numeric_limits<size_t>::max());
                return N.get_ui();
        }
