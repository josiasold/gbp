 ...
 *
 *  \tparam T Should be a scalar that is castable from and to gbp::Real and should support elementary arithmetic operations.
 */
        template <typename T>
        class TProb
        {
         ...
         */
                this_type normalized(ProbNormType norm = gbp::NORMPROB) const
                {
                        T Z = 0;
                        if (norm == gbp::NORMPROB)
                                Z = sum();
 ...
 */
        ...
        /// Represents a vector with entries of type gbp::Real.
