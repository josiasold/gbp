 ...
 *  corresponding to joint states of variables to and from the canonical ordering used in libGBP.
 */
        class Permute
        {
        private:
        ...
        public:
                ...
                {
                        GBP_ASSERT(_ranges.size() == _sigma.size());
                }
 ...
 *  \endcode
 *
 *  \note The same functionality could be achieved by simply iterating over the linear state and using gbp::calcState(),
 *  but the State class offers a more efficient implementation.
 *
 *  \note A State is very similar to a gbp::multifor, but tailored for Var 's and VarSet 's.
 *
 *  \see gbp::calcLinearState(), gbp::calcState()
 *
 *  \idea Make the State class a more prominent part of libGBP (and document it clearly, explaining the concept of state); 
