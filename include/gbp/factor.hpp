...
#ifndef __defined_libGBP_factor_h
#define __defined_libGBP_factor_h
 ...
 *
 *  In libGBP, a factor is represented by a TFactor<T> object, which has two
 *  components:
 ...
 *  induced by gbp::calcLinearState().
 *
 ...
 */
         ...
         */
                ...
                {
                        GBP_ASSERT((BigInt)x.size() == vars.nrStates());
         ...
         */
                ...
                {
                        GBP_ASSERT(_vs.nrStates() == (BigInt)_p.size());
                }
         ...
         */
                ...
                /// Returns argmax of Factor, obtained by maximizing all variables
                std::map<gbp::Var, size_t> argMax() const;
                /// Returns 2nd argmax of Factor, obtained by maximizing all variables
                std::map<gbp::Var, size_t> argMax2() const;
                //@}
        };
        ...
        template <typename T>
        std::map<gbp::Var, size_t> TFactor<T>::argMax() const
        {
                std::map<gbp::Var, size_t> res;
                long double max = 0;
                for (size_t linearState = 0; linearState < _p.size(); linearState++)
                {
                        std::map<gbp::Var, size_t> states = gbp::calcState(_vs, linearState);
                        if (_p.get(linearState) >= max)
                        {
                                res = states;
                                max = _p.get(linearState);
                        }
                }
                return res;
        }
        ...
        template <typename T>
        std::map<gbp::Var, size_t> TFactor<T>::argMax2() const
        {
                std::map<gbp::Var, size_t> res;
                std::map<gbp::Var, size_t> res2;
                long double max = 0;
                long double max2 = 0;
                for (size_t linearState = 0; linearState < _p.size(); linearState++)
                {
        ...
        {
                GBP_DEBASSERT(_vs.contains(i));
                GBP_DEBASSERT(_vs.contains(j));
                GBP_DEBASSERT(i != j);
                VarSet ij(i, j);
 ...
 */
        ...
        /// Represents a factor with values of type gbp::Real.
