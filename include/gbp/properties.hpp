    ...
    PropertySet p()("method","BP")("verbose",1)("tol",1e-9)
            ...
            */
            ...
            PropertySet& setAsString( const PropertyKey& key, const ValueType& val ) {
                try {
                    ...
                    GBP_THROWE(IMPOSSIBLE_TYPECAST,"Cannot cast value of property '" + key + "' to string.");
                }
            }
            ...
            void convertTo( const PropertyKey& key ) { 
                ...
                if( val.type() != typeid(ValueType) ) {
                    GBP_ASSERT( val.type() == typeid(std::string) );
                    try {
                        set(key, boost::lexical_cast<ValueType>(getAs<std::string>(key)));
                    } catch(boost::bad_lexical_cast &) {
                        GBP_THROWE(IMPOSSIBLE_TYPECAST,"Cannot cast value of property '" + key + "' from string to desired type.");
                    }
                }
            }
        //@}
        ...
        //@{
            ...
            const PropertyValue& get( const PropertyKey& key ) const {
                PropertySet::const_iterator x = find(key);
                if( x == this->end() )
                    GBP_THROWE(OBJECT_NOT_FOUND,"PropertySet::get cannot find property '" + key + "'");
                return x->second;
            }
            ...
            ValueType getAs( const PropertyKey& key ) const {
                try {
                    ...
                    GBP_THROWE(IMPOSSIBLE_TYPECAST,"Cannot cast value of property '" + key + "' to desired type.");
                    return ValueType();
                }
            }
